...
subtitle: A reproducible document
...
# make sure the data file and this markdown are in the same folder
...
Table showing sample characteristics. Maybe add some text here pointing out anything interesting or unexpected in the table that the readers might want to view more closely. Also explain anything in the table that suggests how the research question might eventually be answered.
...
# checking distributions for age and LADDER variables
...
Table. Characteristics and opinions of male and female survey participants from Kenya, Nigeria, South Africa, and the U.S. (n = 5,000).

```{r}
# hide the code chunk but show the table
# table with all the variables of interest
# stratified by country
table1(~ AGE + SEX + LADDER_NOW + BETTER_GENDER + COUNTRY_SATIS + ECON_SIT | country,
...
There would be a paragraph here pointing out any descriptives in the table that might be of interest or useful in helping to answer the research question. You could do this in a paragraph or, if there are several things to point out, maybe a bulleted list would be useful.  
...
Figure. Satisfaction with how things are going by country according to 5001 participants in a 2019 survey of people from Kenya, Nigeria, South Africa, and the United States.
...
Figure. Life ladder by country for 5001 participants in a 2019 survey of people from Kenya, Nigeria, South Africa, and the United States.

```{r}
# hide the code chunk but show the figure
# Life ladder by sex
globalData2019clean %>% 
  ggplot(aes(y = LADDER_NOW, x = country, fill = country)) +
...
Figure. Who has a better life by country according to 5001 participants in a 2019 survey of people from Kenya, Nigeria, South Africa, and the United States.
